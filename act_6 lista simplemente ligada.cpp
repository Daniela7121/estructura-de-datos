...
public:
    ...
    friend ostream& operator<<(ostream& os, const SocioClub& s) {
        os << "Numero de socio: " << s.numeroSocio << "\n"
...
public:
    ...
    void registrarSocio(const SocioClub& s) {
        ...
        for (Nodo* actual = cabeza; actual; actual = actual->siguiente) {
            if (actual->socio == s) {
                cerr << "Error: Ya existe un socio con ese numero." << endl;
                return;
            }
        }
    ...
    void darDeBaja(int numero) {
        ...
        if (actual->siguiente) {
            ...
            cerr << "No se encontro el socio con ese numero." << endl;
        }
    }
        ...
        try {
            switch (opcion) {
                case 1: {
                    ...
                    if (!(cin >> numero)) throw runtime_error("Numero de socio invalido");
                    cin.ignore();

                    cout << "Nombre: ";
                    getline(cin, nombre);
                    if (nombre.empty()) throw runtime_error("Nombre no puede estar vacio");

                    cout << "Domicilio: ";
                    getline(cin, domicilio);
                    if (domicilio.empty()) throw runtime_error("Domicilio no puede estar vacio");

                    cout << "Año de ingreso: ";
                    if (!(cin >> ano)) throw runtime_error("Año de ingreso invalido");
                    cin.ignore();
                ...
                case 2: {
                    int numero;
                    cout << "Numero de socio a dar de baja: ";
                    if (!(cin >> numero)) throw runtime_error("Numero de socio invalido");
                    cin.ignore();
